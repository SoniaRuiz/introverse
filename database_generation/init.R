...
init_recount3_gtex_data <- function (projects_used,
  ...
  if ( file.exists(paste0("./database/all_split_reads_raw.rds")) ) {
    
    print(paste0(Sys.time(), " - loading 'all_split_reads_raw.rds' file..."))
    all_split_reads_raw_tidy <- readRDS(file = paste0("./database/all_split_reads_raw.rds"))
  ...
  folder_path <- paste0("./database/v", gtf_version, "/")
  dir.create(file.path(folder_path), recursive = TRUE, showWarnings = T)
...
tidy_data_pior_sql <- function (projects_used, 
  ...
  all_split_reads_details_w_symbol_reduced_keep_gr <- readRDS(file = paste0("./database/v",
                                                                            gtf_version, 
                                                                            "/all_split_reads_details_w_symbol_reduced_keep.rds"))
  ...
  all_split_reads_details <- readRDS(file = paste0("./database/v",
  ...
  setdiff(all_split_reads_details_w_symbol_reduced_keep_gr$junID, 
          all_split_reads_details$junID) %>% unique %>% length()
  # ## Stats
  ...
  df_all_distances_pairings_raw <- readRDS(file = paste0("./database/v",
                                                         gtf_version, "/",
