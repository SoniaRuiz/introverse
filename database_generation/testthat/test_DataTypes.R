...
context("\tTest that MANE table data types and foreign keys are correctly set up")
test_that("Test that MANE table data types and foreign keys are correctly set up", {
  con <- DBI::dbConnect(RSQLite::SQLite(), database_path)
  
  mane_table <- getTableInformation(con, "mane")
  mane_table_types <- mane_table %>% select(name, type) %>% tibble::deframe()
  mane_table_notnull <- mane_table %>% select(name, notnull) %>% tibble::deframe()
  mane_table_pk <- mane_table %>% select(name, pk) %>% tibble::deframe()
  
  expect_equal(mane_expected_types, mane_table_types)
  expect_equal(mane_expected_notnull, mane_table_notnull)
  expect_equal(mane_expected_pk, mane_table_pk)
  DBI::dbDisconnect(con)
})
...
test_that("Test that cluster mis-spliced tables data types and foreign keys are correctly set up", {
  ...
  for(table in misspliced_tables){
    ...
    misspliced_expected_foreign <- rbind(misspliced_expected_foreign_gene_id, misspliced_expected_foreign_novel_junID,
