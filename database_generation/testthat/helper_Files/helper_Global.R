...
main_path <- "./"
database_path <- paste0(getwd(), "/../introverse/dependencies/introverse.sqlite") #paste0("~/PROJECTS/introverse-app/introverse/dependencies/introverse.sqlite") 
...
### Clusters to test
#test_clusters <- "all" # Input a list of IntroVerse database tables' name such as c("Adipose - Subcutaneous_ADIPOSE_TISSUE", "Brain - Frontal Cortex (BA9)_BRAIN")
test_clusters <- c("Adipose - Subcutaneous_ADIPOSE_TISSUE")
num_cores <- 8

#################### Dependencies
## Additional Files
reference_transcriptome_path <- paste0(main_path, "/dependencies/Homo_sapiens.GRCh38.105.chr.gtf")
clinvar_path <-  paste0(main_path, "/dependencies/clinvar_intronic_tidy.rda")
...
fasta_path <- path.expand("/home/grocamora/RytenLab-Research/Additional_files/Homo_sapiens.GRCh38.dna.primary_assembly.fa")
fordownload_path <- path.expand("/home/grocamora/tools/fordownload/")
# bedtools_path <- path.expand(paste0(main_path, "/dependencies/tools/bedtools/"))
# fasta_path <- path.expand(paste0(main_path, "/dependencies/Homo_sapiens.GRCh38.dna.primary_assembly.fa"))
# fordownload_path <- path.expand(paste0(main_path, "/dependencies/tools/fordownload/"))

#################### Variables to validate the data
### Database tables:
expected_main_tables = c("intron", "novel", "gene", "master", "mane") %>% sort()
### Intron table:
...
#'   project.
#' @param results_path Path to the results containing each project's folder.
...
generateClusterSplitReadsPath <- function(table_name, results_path, gtf_version, main_project){
  ...
  cluster_split_reads_path <- paste0(results_path, project, "/v", gtf_version, "/", 
                                     main_project, "_project/", "base_data/", 
                                     project, "_", cluster, "_split_read_counts.rds")
...
generateTPMpath <- function(table_name, results_path, gtf_version, main_project){
  ...
  tpm_path <- paste0(results_path, project, "/v", gtf_version, "/", 
                     main_project, "_project/results/tpm/", 
                     project, "_", cluster, "_tpm.rds")
  return(tpm_path)
}
